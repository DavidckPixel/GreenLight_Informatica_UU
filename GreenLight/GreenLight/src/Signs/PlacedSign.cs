...
namespace GreenLight
{
    public class PlacedSign : AbstractSign
    {
        ...
        public void setLocation()
        {
            ...
            Point hitbox = new Point(-100, -100);
            Point hitboxoff = new Point(-100, -100);

            try
            {
                List<LanePoints> _lanepoints = this.Road.Drivinglanes[_outerLane].points;
                List<LanePoints> _midLane = this.Road.Drivinglanes[this.Road.lanes / 2].points;
                ...
                for (int i = 0; i < _lanepoints.Count; i++)
                {
                    ...
                    if (_shortDistance > _distance)
                    {
                        ...
                        hitbox = _midLane[i].cord;
                        if (i - 10 >= 0) {
                            hitboxoff = _midLane[i - 10].cord;
                            h = _lanepoints[i - 10].cord;
                        }
                        if (i - 20 >= 0)
                        {
                            h = _lanepoints[i - 20].cord;
                            hitboxoff = _midLane[i - 20].cord;
                        }
                        else
                        {
                            h = _lanepoints[i].cord;
                            hitboxoff = _midLane[i].cord;
                        }
                    }
                }
            ...
            if (!flipped)
            {
                _points = RoadMath.hitBoxPointsDiagonal(hitbox, hitboxoff, this.Road.lanes + 2, 20, true, RoadMath.calculateSlope(this.Location, this.Hitboxoffset));
            }
            else
            {
                _points = RoadMath.hitBoxPointsDiagonal(hitbox, hitboxoff, this.Road.lanes + 2, 20, true, RoadMath.calculateSlope(this.Hitboxoffset, this.Location));
            }
